...
void main() {
  ...
  setUp(() {
    ...
    repository = OnboardingRepositoryImpl(preferencesRepository: preferences);
  });
  ...
  group('OnboardingRepositoryImpl', () {
    group('hasSeenOnboarding', () {
      test('returns false when flag is absent', () {
        when(
          () => preferences.getBool(key: any(named: 'key')),
        ).thenReturn(null);
      ...
      test('delegates to PreferencesRepository with correct key', () {
        ...
        verify(() => preferences.getBool(key: 'onboarding_seen')).called(1);
      });
    });

    group('markOnboardingSeen', () {
      test(
        'delegates to PreferencesRepository with correct key and value',
        () async {
          when(
            () => preferences.setBool(
              key: any(named: 'key'),
